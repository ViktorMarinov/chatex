defmodule ChatexClient.InputReader do
  ...
  alias ChatexClient.InputReader.InChannel
  ...
  def start(username, key_phrase) do
    ...
    Connector.connect(username, key_phrase)
  ...
  defp handle_connect({:error, :unregistered}, {username, key_phrase}) do
    ...
    case Connector.register(username, key_phrase) do
  ...
  defp handle_command({:ok, :exit}), do: IO.puts("Stopping Chatex")
  defp handle_command(command) do
    case command do
      ...
      {:ok, {:open_chat, to_user}} ->
        ...
        InChat.listen(to_user)
      {:ok, {:join_channel, channel_name}} -> 
        IO.puts("Joining channel #{channel_name}")
        InChannel.listen(channel_name)
      {:ok, command} -> Connector.call_command(command)
    end
